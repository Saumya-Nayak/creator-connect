...
function applyUrlFilters() {
  ...
  if (!role) return;
  if (role === "buyer" || role === "seller") switchRole(role);
  if (type && type !== "all") filterByType(role, type);
  if (status) filterByStatus(role, status);
...
function switchRole(role) {
  ...
  document.getElementById("sellerStatusFilter").style.display =
    role === "seller" ? "flex" : "none";
  document
    .querySelectorAll(".deals-section")
...
function getStatusCount(role, type, status) {
  if (type === "all") {
    const all = [...allDeals[role].products, ...allDeals[role].services];
    return status === "all"
      ? all.length
...
async function loadAllDeals() {
  ...
  try {
    const token =
      localStorage.getItem("authToken") || sessionStorage.getItem("authToken");
    const [buyerProducts, buyerServices, sellerProducts, sellerServices] =
      await Promise.all([
...
function updateQuickStats() {
  ...
  document.getElementById("activeOrders").textContent = all.filter(
    (d) =>
      !["delivered", "completed", "cancelled", "rejected"].includes(d.status)
  ).length;
  document.getElementById("completedOrders").textContent = all.filter((d) =>
...
function renderDeals() {
  ...
  let deals =
    type === "all"
      ? [...allDeals[role].products, ...allDeals[role].services]
      : allDeals[role][type];

  if (status !== "all") deals = deals.filter((d) => d.status === status);
  deals.sort(
    (a, b) =>
      new Date(b.order_date || b.booking_date || 0) -
      new Date(a.order_date || a.booking_date || 0)
  );
  ...
  deals.forEach((deal) => {
    ...
    dealsGrid.appendChild(
      isProduct ? createProductCard(deal, role) : createServiceCard(deal, role)
    );
  });
}
...
function _isServiceExpired(booking) {
  ...
  if (slotLabel && slotLabel.includes(":")) {
    ...
    expiryMs = new Date(booking.preferred_start_date + "T23:59:59").getTime();
  }
...
// createProductCard and createServiceCard — unchanged from original, keeping full code
function createProductCard(order, role) {
  ...
  const rejectionInfo =
    order.status === "cancelled" && order.cancellation_reason
      ...
      : "";
  const overdueBadge = _isProductOverdue(order)
    ? `<span style="display:inline-flex;align-items:center;gap:5px;background:rgba(239,68,68,.1);color:#dc2626;border:1px solid rgba(239,68,68,.35);border-radius:20px;padding:3px 10px;font-size:.75rem;font-weight:700;margin-top:4px"><i class="fas fa-exclamation-circle"></i> Delivery Overdue</span>`
    : "";
  const isPickup = Boolean(order.is_pickup);
  const fulfillmentBadge = isPickup
    ? `<span style="display:inline-flex;align-items:center;gap:5px;background:rgba(139,92,246,0.1);color:#7c3aed;font-size:.73rem;font-weight:600;padding:3px 8px;border-radius:20px;border:1px solid rgba(139,92,246,0.2);margin-top:4px"><i class="fas fa-store" style="font-size:.65rem"></i> Pickup Order</span>`
    : "";
  ...
  card.innerHTML = `
    <div class="deal-card-image">
      ${
        imageUrl
          ? isProductVideo
            ? `<div style="position:relative;width:100%;height:100%;"><video class="deals-autoplay-video" muted loop playsinline preload="metadata" style="width:100%;height:100%;object-fit:cover;display:block;"><source src="${imageUrl}" type="video/mp4"></video><button onclick="event.stopPropagation();toggleDealsVideoMute(event,this)" style="position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.55);color:white;border:none;border-radius:50%;width:28px;height:28px;font-size:0.75rem;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:5;"><i class="fas fa-volume-mute"></i></button></div>`
      ...
      <span class="deal-type-badge product grid-image-badge"><i class="fas fa-box"></i> Product</span>
      ${
        isCod
    ...
    <div class="deal-card-body">
      <div class="deal-card-header">
        <div class="deal-info">
          <span class="deal-type-badge product list-body-badge"><i class="fas fa-box"></i> Product</span>
          ${codBadge}
          ...
          ${fulfillmentBadge}${overdueBadge}${rejectionInfo}
        </div>
        <div class="deal-status-container">
          <span class="status-badge ${order.status}">${getStatusIcon(
    order.status
  )} ${statusLabel}</span>
...
function createServiceCard(booking, role) {
  ...
  const expiryBadge = expired
    ? `<span style="display:inline-flex;align-items:center;gap:5px;background:rgba(245,158,11,.12);color:#b45309;border:1px solid rgba(245,158,11,.4);border-radius:20px;padding:3px 10px;font-size:.75rem;font-weight:700;margin-top:4px"><i class="fas fa-hourglass-end"></i> Service Time Passed</span>`
    : "";
  const slotLabel =
    booking.booked_slot ||
    (booking.preferred_time
  ...
  card.innerHTML = `
    <div class="deal-card-image">
      ...
      <span class="deal-type-badge service grid-image-badge"><i class="fas fa-briefcase"></i> Service</span>
    </div>
    <div class="deal-card-body">
      <div class="deal-card-header">
        <div class="deal-info">
          <span class="deal-type-badge service list-body-badge"><i class="fas fa-briefcase"></i> Service</span>
          ...
          ${expiryBadge}${providerMessageInfo}${cancellationInfo}
        </div>
        <div class="deal-status-container">
          <span class="status-badge ${booking.status}">${getStatusIcon(
    booking.status
...
function createProductActions(order, role) {
  ...
  if (role === "buyer") {
    if (["pending", "confirmed"].includes(order.status))
      actions.push(
        `<button class="btn btn-danger" data-action="cancel-order" data-order-id="${order.order_id}" onclick="setButtonLoading(this,'Opening...')"><i class="fas fa-times"></i> Cancel Order</button>`
      );
    const canPay =
    ...
    if (canPay)
      actions.push(
        `<button class="btn btn-warning" data-action="make-payment" data-order-id="${
          ...
          order.accepts_cod || 0
        }"><i class="fas fa-credit-card"></i> Make Payment</button>`
      );
    actions.push(
      `<button class="btn btn-secondary" data-action="view-order-details" data-order-id="${order.order_id}"><i class="fas fa-eye"></i> View Details</button>`
    );
    if (
      ...
      actions.push(
        `<button class="btn btn-secondary" data-action="change-order-details" data-order-id="${order.order_id}"><i class="fas fa-edit"></i> Change Details</button>`
      );
  } else {
    if (order.status === "pending") {
      actions.push(
        `<button class="btn btn-success" data-action="confirm-order" data-order-id="${order.order_id}" onclick="setButtonLoading(this,'Confirming...')"><i class="fas fa-check"></i> Confirm Order</button>`
      );
      actions.push(
        `<button class="btn btn-danger" data-action="reject-order" data-order-id="${order.order_id}" onclick="setButtonLoading(this,'Opening...')"><i class="fas fa-times"></i> Reject Order</button>`
      );
    }
    if (["confirmed", "processing", "shipped"].includes(order.status))
      actions.push(
        `<button class="btn btn-primary" data-action="update-order-status" data-order-id="${order.order_id}"><i class="fas fa-sync"></i> Update Status</button>`
      );
    if (order.payment_status === "completed")
      actions.push(
        `<button class="btn btn-success" onclick="window.location.href='settings.html?tab=seller'"><i class="fas fa-money-bill-transfer"></i> Withdraw</button>`
      );
    actions.push(
      `<button class="btn btn-secondary" data-action="view-order-details" data-order-id="${order.order_id}"><i class="fas fa-eye"></i> View Details</button>`
    );
...
function createServiceActions(booking, role) {
  ...
  if (role === "buyer") {
    if (["pending", "accepted"].includes(booking.status))
      actions.push(
        `<button class="btn btn-danger" data-action="cancel-booking" data-booking-id="${booking.booking_id}" onclick="setButtonLoading(this,'Cancelling...')"><i class="fas fa-times"></i> Cancel Booking</button>`
      );
    if (booking.status === "accepted" || booking.status === "in_progress") {
      ...
      if (contactHref)
        actions.push(
          `<a class="btn btn-success" href="${contactHref}" target="_blank" rel="noopener noreferrer"><i class="${getContactIcon(
            booking.contact_method
          )}"></i> Contact Provider</a>`
        );
    }
    actions.push(
      `<button class="btn btn-secondary" data-action="view-booking-details" data-booking-id="${booking.booking_id}"><i class="fas fa-eye"></i> View Details</button>`
    );
  } else {
    if (booking.status === "pending") {
      actions.push(
        `<button class="btn btn-success" data-action="accept-booking" data-booking-id="${booking.booking_id}" onclick="setButtonLoading(this,'Accepting...')"><i class="fas fa-check"></i> Accept Booking</button>`
      );
      actions.push(
        `<button class="btn btn-danger" data-action="reject-booking" data-booking-id="${booking.booking_id}" onclick="setButtonLoading(this,'Opening...')"><i class="fas fa-times"></i> Reject Booking</button>`
      );
    }
    if (["accepted", "in_progress"].includes(booking.status)) {
      actions.push(
        `<button class="btn btn-primary" data-action="update-booking-status" data-booking-id="${booking.booking_id}" onclick="setButtonLoading(this,'Loading...')"><i class="fas fa-sync"></i> Update Status</button>`
      );
      const contactHref = buildContactHref(
        booking.contact_method,
  ...
  switch ((method || "email").toLowerCase()) {
    case "whatsapp":
      return `https://wa.me/${clean.replace(
        /[^\d+]/g,
        ""
      )}?text=${encodeURIComponent(`Hi! I'm reaching out about: "${title}"`)}`;
    case "phone":
      return `tel:${clean}`;
    default:
...
function getContactIcon(method) {
  return (
    ...
    }[(method || "email").toLowerCase()] || "fas fa-envelope"
  );
}
...
// ===== ✅ FIXED: updateOrderStatus — correct endpoint =====
async function updateOrderStatus(orderId, status, additionalData = {}) {
  ...
  try {
    ...
    // ✅ FIX: Use the correct booking_routes endpoint
    const response = await fetch(
      `${API_BASE_URL}/product-orders/${orderId}/status`,
      {
        method: "PUT",
        headers: {
...
// ===== ✅ FIXED: updateBookingStatus — correct endpoint =====
async function updateBookingStatus(bookingId, status, additionalData = {}) {
  ...
  try {
    ...
    if (status === "accepted" || status === "rejected") {
      endpoint = `${API_BASE_URL}/service-bookings/${bookingId}/respond`;
      additionalData = {
        action: status === "accepted" ? "accept" : "reject",
        message: additionalData.provider_message || "",
      };
    } else if (status === "cancelled") {
      endpoint = `${API_BASE_URL}/service-bookings/${bookingId}/cancel`;
      additionalData = { reason: additionalData.cancellation_reason || "" };
    } else {
      // in_progress, completed — use the deals endpoint (your deals_routes.py)
      endpoint = `${API_BASE_URL}/deals/bookings/${bookingId}/status`;
    }
...
// ===== ✅ FIXED: confirmOrder — correct endpoint =====
async function confirmOrder(orderId) {
  ...
  try {
    ...
    // ✅ FIX: Use the correct booking_routes endpoint
    const response = await fetch(
      `${API_BASE_URL}/product-orders/${orderId}/confirm`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
// ===== ✅ FIXED: acceptBooking — correct endpoint =====
async function acceptBooking(bookingId) {
  const btn = document.querySelector(
    `[data-action="accept-booking"][data-booking-id="${bookingId}"]`
  );
  if (btn) {
    btn.disabled = true;
  ...
  try {
    ...
    // ✅ FIX: Use the correct booking_routes endpoint
    const response = await fetch(
      `${API_BASE_URL}/service-bookings/${bookingId}/respond`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    if (btn && document.contains(btn)) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-check"></i> Accept Booking';
      btn.style.opacity = "1";
    }
  }
}

// ===== CANCEL / REJECT MODALS =====
function showCancelOrderModal(orderId) {
  showModal(
    "Cancel Order",
    `<div class="form-group"><label>Reason for cancellation (optional)</label><textarea id="cancelReason" placeholder="Please provide a reason..." style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);"></textarea></div>`,
    `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-danger" data-action="execute-cancel-order" data-id="${orderId}"><i class="fas fa-times"></i> Confirm Cancellation</button>`
  );
}
async function executeCancelOrder(orderId) {
  ...
  try {
    const token =
      localStorage.getItem("authToken") || sessionStorage.getItem("authToken");
    const response = await fetch(
      `${API_BASE_URL}/product-orders/${orderId}/cancel`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    if (data.success) {
      ...
      await loadAllDeals();
      closeModal();
    } else showToast(data.message || "Failed to cancel order", "error");
  } catch (e) {
...
function showRejectOrderModal(orderId) {
  showModal(
    "Reject Order",
    `<div class="form-group"><label>Reason for rejection *</label><textarea id="rejectReason" placeholder="Please provide a reason..." required style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);"></textarea></div>`,
    `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-danger" data-action="execute-reject-order" data-id="${orderId}"><i class="fas fa-times"></i> Confirm Rejection</button>`
  );
}
async function executeRejectOrder(orderId) {
  ...
  try {
    const token =
      localStorage.getItem("authToken") || sessionStorage.getItem("authToken");
    const response = await fetch(
      `${API_BASE_URL}/product-orders/${orderId}/reject`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    if (data.success) {
      ...
      await loadAllDeals();
      closeModal();
    } else showToast(data.message || "Failed to reject order", "error");
  } catch (e) {
    showToast(e.message || "Failed to reject order", "error");
  } finally {
    setModalButtonLoading(false);
  }
}
...
function showUpdateOrderStatusModal(orderId) {
  const order = allDeals.seller.products.find((o) => o.order_id == orderId);
  if (!order) return;
  const isPickup = Boolean(order.is_pickup);
  ...
  const codNote =
    nextStatus.value === "delivered" && order.payment_method === "cod"
      ? `<div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:10px;padding:12px;margin-top:16px;font-size:.88rem"><i class="fas fa-info-circle" style="color:#22c55e;margin-right:6px"></i><strong>COD Order:</strong> ${
          isPickup ? "Buyer pays on pickup." : "Buyer pays on delivery."
        } A 2% commission (₹${formatMoney(
          order.total_amount * 0.02
        )}) will be deducted.</div>`
      : "";
  const trackingField =
    nextStatus.value === "shipped" && !isPickup
      ? `<div class="form-group"><label>Tracking Number (optional)</label><input type="text" id="trackingNumber" placeholder="Enter tracking number..." style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);"></div>`
      : "";
  showModal(
    "Update Order Status",
    `<div style="text-align:center;padding:20px;"><i class="fas ${nextStatus.icon}" style="font-size:4rem;color:var(--primary-purple);margin-bottom:20px;"></i><h3 style="margin-bottom:10px;">${nextStatus.label}</h3><p style="color:var(--text-secondary);margin-bottom:20px;">Update the order status to inform the buyer</p>${trackingField}${codNote}</div>`,
    `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-primary" data-action="execute-update-status" data-id="${orderId}" data-entity-type="order"><i class="fas fa-sync"></i> Update Status</button>`
  );
}
...
async function executeUpdateStatus(id, entityType) {
  if (entityType === "order") {
    ...
    const isPickup = Boolean(order.is_pickup);
    const nextStatuses = isPickup
      ? { confirmed: "processing", processing: "delivered" }
      : {
    ...
    const booking = allDeals.seller.services.find((b) => b.booking_id == id);
...
function showRejectBookingModal(bookingId) {
  showModal(
    "Reject Booking",
    `<div class="form-group"><label>Reason for rejection *</label><textarea id="rejectReason" placeholder="Please provide a reason..." required style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);"></textarea></div>`,
    `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-danger" data-action="execute-reject-booking" data-id="${bookingId}"><i class="fas fa-times"></i> Confirm Rejection</button>`
  );
}
...
function showCancelBookingModal(bookingId) {
  showModal(
    "Cancel Booking",
    `<div class="form-group"><label>Reason for cancellation (optional)</label><textarea id="cancelReason" placeholder="Please provide a reason..." style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);"></textarea></div>`,
    `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-danger" data-action="execute-cancel-booking" data-id="${bookingId}"><i class="fas fa-times"></i> Confirm Cancellation</button>`
  );
}
...
function showUpdateBookingStatusModal(bookingId) {
  ...
  showModal(
    ...
    `<div style="text-align:center;padding:20px;"><i class="fas ${nextStatus.icon}" style="font-size:4rem;color:var(--primary-purple);margin-bottom:20px;"></i><h3 style="margin-bottom:10px;">${nextStatus.label}</h3><p style="color:var(--text-secondary);margin-bottom:20px;">Update the booking status to inform the customer</p></div>`,
    `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-primary" data-action="execute-update-status" data-id="${bookingId}" data-entity-type="booking"><i class="fas fa-sync"></i> Update Status</button>`
  );
}
...
function showPaymentModal(orderId, amount) {
  ...
  const codOption = acceptsCod
    ...
    : "";
  const bodyHTML = `<div style="padding:4px 0 20px;">
    <div style="background:var(--light-purple);padding:18px;border-radius:14px;text-align:center;margin-bottom:20px;">
      <div style="font-size:.85rem;color:var(--text-secondary);margin-bottom:4px;">Amount to Pay</div>
      <div style="font-size:2.2rem;font-weight:800;color:var(--primary-purple);">₹${formatMoney(
        amount
      )}</div>
    </div>
    <select id="paymentMethod" onchange="handlePaymentMethodChange(this.value)" style="width:100%;padding:12px 14px;border-radius:10px;border:2px solid var(--border-purple);font-size:.95rem;margin-bottom:20px;background:var(--bg,#fff);color:inherit;">
      <option value="upi">📱 UPI (Recommended)</option>
      <option value="bank_transfer">🏦 Bank Transfer / NEFT</option>
      ${codOption}
    </select>
    <div id="paymentDetails-upi" style="background:rgba(99,102,241,.07);border:1.5px solid rgba(99,102,241,.2);border-radius:12px;padding:16px;margin-bottom:16px;">
      <div style="display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap;">
        <div style="flex-shrink:0;text-align:center;">
          <div id="upiQrCode" style="width:130px;height:130px;background:#f3f4f6;border-radius:10px;display:flex;align-items:center;justify-content:center;"><i class="fas fa-circle-notch fa-spin" style="color:#ccc;font-size:2rem;"></i></div>
          <div style="font-size:.7rem;color:var(--text-secondary);margin-top:4px;">Scan & Pay</div>
        </div>
        <div style="flex:1;min-width:160px;">
          <div style="font-size:.8rem;color:var(--text-secondary);">UPI ID</div>
          <div style="font-size:1.1rem;font-weight:800;margin-bottom:4px;">${escapeHtml(
            upiId
          )}</div>
          <div style="font-size:.85rem;color:var(--text-secondary);margin-bottom:12px;">${escapeHtml(
            upiName
          )}</div>
          <button onclick="navigator.clipboard.writeText('${escapeHtml(
            upiId
          )}').then(()=>showToast('UPI ID copied!','success'))" style="padding:6px 14px;border-radius:8px;border:1.5px solid var(--border-purple);background:transparent;color:var(--primary-purple);font-weight:600;cursor:pointer;font-size:.8rem;"><i class="fas fa-copy"></i> Copy</button>
        </div>
      </div>
      <div style="font-size:.82rem;color:var(--text-secondary);margin-top:12px;">${escapeHtml(
        upiDesc
      )}</div>
    </div>
    <div id="paymentDetails-bank_transfer" style="display:none;background:rgba(34,197,94,.07);border:1.5px solid rgba(34,197,94,.2);border-radius:12px;padding:16px;margin-bottom:16px;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
        <div><div style="font-size:.75rem;color:var(--text-secondary);">Bank Name</div><div style="font-weight:700;">${escapeHtml(
          bankName
        )}</div></div>
        <div><div style="font-size:.75rem;color:var(--text-secondary);">Account Holder</div><div style="font-weight:700;">${escapeHtml(
          bankHolder
        )}</div></div>
        <div><div style="font-size:.75rem;color:var(--text-secondary);">Account Number</div><div style="font-weight:700;">${escapeHtml(
          bankAccount
        )} <button onclick="navigator.clipboard.writeText('${escapeHtml(
    bankAccount
  )}').then(()=>showToast('Copied!','success'))" style="padding:3px 8px;border-radius:6px;border:1px solid #ccc;background:transparent;cursor:pointer;font-size:.72rem;"><i class="fas fa-copy"></i></button></div></div>
        <div><div style="font-size:.75rem;color:var(--text-secondary);">IFSC</div><div style="font-weight:700;">${escapeHtml(
          bankIfsc
        )}</div></div>
        <div style="grid-column:1/-1;"><div style="font-size:.75rem;color:var(--text-secondary);">Branch</div><div style="font-weight:700;">${escapeHtml(
          bankBranch
        )}</div></div>
      </div>
      <div style="font-size:.82rem;color:var(--text-secondary);">${escapeHtml(
        bankDesc
      )}</div>
    </div>
    <div id="paymentDetails-cod" style="display:none;background:rgba(245,158,11,.08);border:1.5px solid rgba(245,158,11,.3);border-radius:12px;padding:16px;margin-bottom:16px;">
      <div style="font-size:1.5rem;margin-bottom:8px;">💵</div>
      <div style="font-weight:700;margin-bottom:6px;">Cash on Delivery</div>
      <ul style="font-size:.85rem;color:var(--text-secondary);padding-left:18px;line-height:1.8;"><li>No online payment required now</li><li>Pay cash directly to the delivery person / seller</li><li>A <strong>${codPct}% platform fee</strong> is deducted from the seller's earnings</li></ul>
    </div>
    <div id="step3Wrap">
      <input type="text" id="paymentReferenceBuyer" placeholder="UPI Reference / UTR / NEFT Ref No. (optional)" style="width:100%;padding:12px 14px;border-radius:10px;border:2px solid var(--border-purple);font-size:.9rem;margin-bottom:16px;">
    </div>
    <div style="background:rgba(99,102,241,.06);border-left:3px solid var(--primary-purple);border-radius:0 10px 10px 0;padding:12px 14px;font-size:.82rem;color:var(--text-secondary);">
      <i class="fas fa-info-circle" style="color:var(--primary-purple);margin-right:6px;"></i>After clicking <strong>"I Have Paid"</strong>, admin will verify your payment.
      <br><br><i class="fas fa-shield-alt" style="color:#22c55e;margin-right:6px;"></i>Platform fee: <strong>${onlinePct}% on UPI/Bank</strong> · <strong>${codPct}% on COD</strong>
    </div>
  </div>`;
  const footerHTML = `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-success" data-action="execute-payment" data-id="${orderId}" id="iHavePaidBtn"><i class="fas fa-check-circle"></i> I Have Paid</button>`;
...
function showChangeOrderDetailsModal(orderId) {
  ...
  showModal(
    "Change Order Details",
    `
    <div class="form-group"><label>Full Name *</label><input type="text" id="shippingFullName" value="${escapeHtml(
      order.shipping_full_name || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);" required></div>
    <div class="form-group"><label>Phone Number *</label><input type="tel" id="shippingPhone" value="${escapeHtml(
      order.shipping_phone || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);" required></div>
    <div class="form-group"><label>Address Line 1 *</label><input type="text" id="shippingAddress1" value="${escapeHtml(
      order.shipping_address_line1 || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);" required></div>
    <div class="form-group"><label>Address Line 2</label><input type="text" id="shippingAddress2" value="${escapeHtml(
      order.shipping_address_line2 || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);"></div>
    <div class="form-group"><label>City *</label><input type="text" id="shippingCity" value="${escapeHtml(
      order.shipping_city || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);" required></div>
    <div class="form-group"><label>State *</label><input type="text" id="shippingState" value="${escapeHtml(
      order.shipping_state || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);" required></div>
    <div class="form-group"><label>Pincode *</label><input type="text" id="shippingPincode" value="${escapeHtml(
      order.shipping_pincode || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);" required></div>
    <div class="form-group"><label>Landmark</label><input type="text" id="shippingLandmark" value="${escapeHtml(
      order.shipping_landmark || ""
    )}" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);"></div>
    <div class="form-group"><label>Special Instructions</label><textarea id="buyerNotes" style="width:100%;padding:12px;border-radius:8px;border:2px solid var(--border-purple);">${escapeHtml(
      order.buyer_notes || ""
    )}</textarea></div>`,
    `<button class="btn btn-secondary" data-action="close-modal">Cancel</button><button class="btn btn-primary" data-action="execute-change-details" data-id="${orderId}"><i class="fas fa-save"></i> Save Changes</button>`
  );
}
...
async function executeChangeOrderDetails(orderId) {
  ...
  try {
    ...
    if (data.success) {
      ...
      await loadAllDeals();
      closeModal();
    } else showToast(data.message || "Failed to update order details", "error");
  } catch (error) {
...
function showOrderDetailsModal(orderId) {
  ...
  if (isPickup && (order.pickup_address || order.pickup_city)) {
    ...
    const mapsUrl = mapsQuery
      ...
      : null;
    addressSection = `<h4 style="color:var(--primary-purple);margin:20px 0 12px 0">🏪 Pickup Address</h4><div class="modal-detail-row">${
      order.pickup_address
    ...
    addressSection = `<h4 style="color:var(--primary-purple);margin:20px 0 12px 0">📦 Shipping Address</h4><div class="modal-detail-row" style="line-height:1.8"><strong>${escapeHtml(
      order.shipping_full_name || ""
    )}</strong><br>${escapeHtml(order.shipping_address_line1 || "")}${
      order.shipping_address_line2
  ...
  const overdueBanner = _isProductOverdue(order)
    ? `<div style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.35);border-radius:10px;padding:10px 14px;margin-bottom:16px;font-size:.85rem;font-weight:600;color:#dc2626;display:flex;align-items:center;gap:8px"><i class="fas fa-exclamation-circle"></i>${
        isPickup
  ...
  const bodyHTML = `<div style="max-height:65vh;overflow-y:auto;padding-right:4px">${overdueBanner}${
    imageUrl
    ...
    ${
      order.buyer_notes
        ? `<h4 style="color:var(--primary-purple);margin:20px 0 12px 0">📝 Special Instructions</h4><div class="modal-detail-row" style="white-space:pre-wrap;line-height:1.6">${escapeHtml(
            order.buyer_notes
          )}</div>`
        : ""
    }
    ${
      order.cancellation_reason
        ? `<h4 style="color:#dc3545;margin:20px 0 12px 0">❌ Cancellation</h4><div class="modal-detail-row"><strong>Reason:</strong> ${escapeHtml(
            order.cancellation_reason
          )}</div>`
        : ""
    }
  ...
  showModal(
    ...
    `<button class="btn btn-secondary" data-action="close-modal">Close</button>${
      showInvoiceBtn
        ? `<button class="btn btn-primary" onclick="generateInvoice('product',${orderId})"><i class="fas fa-file-invoice"></i> View Invoice</button>`
        : ""
    }`
  );
}
...
function showBookingDetailsModal(bookingId) {
  ...
  const slotRow = _slotDisplay
    ? `<div class="modal-detail-row"><strong>Time Slot:</strong> <span style="background:rgba(230,10,234,.1);color:var(--primary-purple);font-weight:700;padding:3px 10px;border-radius:20px;border:1px solid rgba(230,10,234,.3);font-size:.9rem"><i class="fas fa-clock" style="margin-right:4px;font-size:.8rem"></i>${_slotDisplay}</span></div>`
    : "";
  const travelFeeRow =
  ...
  const expiredBanner = _isServiceExpired(booking)
    ? `<div style="background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.4);border-radius:10px;padding:10px 14px;margin-bottom:16px;font-size:.85rem;font-weight:600;color:#92400e;display:flex;align-items:center;gap:8px"><i class="fas fa-hourglass-end"></i> The scheduled service date/time has passed.</div>`
    : "";
  ...
  const bodyHTML = `<div style="max-height:65vh;overflow-y:auto;padding-right:4px">${expiredBanner}${
    imageUrl
      ? `<div style="margin-bottom:20px;border-radius:12px;overflow:hidden"><img src="${imageUrl}" alt="Service" style="width:100%;max-height:200px;object-fit:cover"></div>`
      : ""
  }
    ...
    ${
      booking.customer_requirements
        ? `<h4 style="color:var(--primary-purple);margin:20px 0 12px 0">📝 Requirements</h4><div class="modal-detail-row" style="white-space:pre-wrap;line-height:1.6">${escapeHtml(
            booking.customer_requirements
          )}</div>`
        : ""
    }
    ${
      booking.provider_message
        ? `<h4 style="color:var(--primary-purple);margin:20px 0 12px 0">💬 Provider Message</h4><div class="modal-detail-row">${escapeHtml(
            booking.provider_message
          )}</div>`
        : ""
    }
    ${
      booking.cancellation_reason
        ? `<h4 style="color:#dc3545;margin:20px 0 12px 0">❌ Cancellation</h4><div class="modal-detail-row">${escapeHtml(
            booking.cancellation_reason
          )}</div>`
        : ""
    }
  ...
  showModal(
    ...
    `<button class="btn btn-secondary" data-action="close-modal">Close</button>${
      showInvoiceBtn
        ? `<button class="btn btn-primary" onclick="generateInvoice('service',${bookingId})"><i class="fas fa-file-invoice"></i> View Invoice</button>`
        : ""
    }`
  );
}
...
function generateInvoice(type, id) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Invoice ${invNo}</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:'Segoe UI',Arial,sans-serif;color:#1a1a1a;background:#f5f5f5;}.page{max-width:760px;margin:30px auto;background:white;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,.1);overflow:hidden;}.header{background:linear-gradient(135deg,#e60aea,#9b59b6);padding:32px 40px;color:white;display:flex;justify-content:space-between;align-items:flex-start;}.brand{font-size:1.6rem;font-weight:900;}.body-wrap{padding:36px 40px;}.footer{background:#fafafa;border-top:1px solid #eee;padding:20px 40px;text-align:center;font-size:.78rem;color:#aaa;}@media print{.no-print{display:none!important;}}</style></head>
  <body><div class="page"><div class="header"><div><div class="brand">Creator Connect<small style="display:block;font-size:.75rem;font-weight:400;opacity:.85;margin-top:2px;">Marketplace Invoice</small></div></div><div style="text-align:right"><h2 style="font-size:1.4rem;font-weight:800;">${invNo}</h2><p>${invDate}</p><p>${
    isProduct ? "Product Order" : "Service Booking"
  }</p></div></div>
  <div class="body-wrap"><div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;margin-bottom:32px;"><div><h4 style="font-size:.72rem;text-transform:uppercase;letter-spacing:1px;color:#888;margin-bottom:8px;">Bill To</h4><div style="font-size:1rem;font-weight:700;">${escapeHtml(
    buyerName
  )}</div></div><div><h4 style="font-size:.72rem;text-transform:uppercase;letter-spacing:1px;color:#888;margin-bottom:8px;">${
    isProduct ? "Sold By" : "Service Provider"
  }</h4><div style="font-size:1rem;font-weight:700;">${escapeHtml(
    sellerName
  )}</div></div></div>
  <div style="text-align:center;padding:20px;background:linear-gradient(135deg,rgba(230,10,234,.08),rgba(155,89,182,.08));border-radius:14px;"><div style="font-size:.85rem;color:#888;margin-bottom:4px;">Total Amount</div><div style="font-size:2rem;font-weight:900;color:#e60aea;">₹${formatMoney(
    total
  )}</div></div></div>
  <div class="footer"><p>Thank you for using Creator Connect! — ${invNo}</p><br><button class="no-print" onclick="window.print()" style="padding:8px 24px;background:#e60aea;color:white;border:none;border-radius:8px;cursor:pointer;font-size:.88rem;font-weight:700;">🖨️ Print / Save as PDF</button></div></div></body></html>`;
...
function getImageUrl(mediaUrl) {
  ...
  if (cleanPath.startsWith("uploads/")) {
    if (cleanPath.includes("uploads/posts/"))
      return `${API_BASE_URL}/uploads/${cleanPath.split("uploads/posts/")[1]}`;
...
function formatPaymentStatus(ps, pm) {
  if (ps === "completed") return "✅ Paid";
  if (ps === "verification_pending") return "⏳ Verification Pending";
  if (ps === "failed") return "❌ Payment Rejected";
  if (ps === "cod_pending" || pm === "cod") return "💵 COD — Pay on Delivery";
  if (ps === "pending")
    return pm === "cod" ? "💵 COD — Pay on Delivery" : "⚠️ Payment Pending";
  return ps ? formatStatus(ps) : "—";
}
function getStatusIcon(s) {
  ...
  return icons[s] || '<i class="fas fa-question"></i>';
}
...
function showToast(message, type = "success") {
  ...
  }"></i><span class="toast-message">${message}</span>`;
...
console.log(
  "✅ my-deals.js — FIXED: correct API endpoints for all booking/order actions"
);
